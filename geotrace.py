...
# ================================================================================
# Advanced Tracing Module (интегрировано из geotrace_advanced.py)
...
def trace_worker(target, callback, stop_event, app):
    """
    Улучшенный worker трассировки с использованием AdvancedTracer
    для получения статистики как у PingPlotter (20 зондов на хоп, потери, джиттер)
    """
    L = LANGS[app.lang]
    callback("log", L["log_geo"])

    # Используем улучшенный трассировщик с многократными зондами
    try:
        # Запускаем асинхронную трассировку
        tracer = AdvancedTracer(
            target=target,
            probes=app.trace_probes,      # 20 зондов по умолчанию
            timeout=app.trace_timeout,    # 2 секунды таймаут
            max_ttl=30
        )
        ...
        def run_async_trace():
            ...
            try:
                results = loop.run_until_complete(tracer.trace())
                
                # Определяем целевой IP
                target_ip = tracer.target_ip
                ...
                for stats in results:
                    ...
                    if not stats.ip:
                        continue  # Пропускаем полностью потерянные хопы
                    
                    # Расчет статистики
                    avg_ms = round(stats.avg_rtt, 1) if stats.avg_rtt else None
                    min_ms = round(stats.min_rtt, 1) if stats.min_rtt else None
                    max_ms = round(stats.max_rtt, 1) if stats.max_rtt else None
                    jitter = round(stats.jitter, 2) if stats.jitter else None
                    loss_pct = round(stats.loss_percent, 1)
                    
                    # Получаем геоданные
                    g = get_ip_geo(stats.ip)
                    if g:
                        g.hop = stats.hop
                        g.ms = avg_ms
                        g.ms_min = min_ms
                        g.ms_max = max_ms
                        g.jitter = jitter
                        g.loss_percent = loss_pct
                        g.probes_sent = stats.total_probes
                        g.hidden = stats.is_hidden  # Помечаем скрытые узлы
                        g.anycast = is_anycast(g.asn, g.org)
                        callback("hop", g)
                        
                        # Запуск PTR lookup
                        ptr_result = ptr_city_code(stats.ip)
                        if ptr_result:
                            code, host = ptr_result
                            callback("fix", {"ip": stats.ip, "code": code, "host": host})
                
                # Обнаружение скрытых узлов
                hidden = tracer.detect_hidden_hops()
                if hidden:
                    callback("log", f"Обнаружено скрытых узлов: {len(hidden)}")
                
                callback("done", {"target": target, "global": len(results), "total": len(results), "timeout": sum(1 for s in results if s.losses == s.total_probes)})
                
            except Exception as e:
                callback("error", f"Ошибка трассировки: {e}")
            finally:
                loop.close()
        
        # Запуск в потоке
        thread = threading.Thread(target=run_async_trace, daemon=True)
        thread.start()
        return
        
    except Exception as e:
        # Fallback на старый метод если AdvancedTracer не работает
        callback("log", f"Улучшенная трассировка недоступна: {e}, используем стандартную...")
